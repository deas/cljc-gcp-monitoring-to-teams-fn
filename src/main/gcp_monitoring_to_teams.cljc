...
(defn gcp-to-teams [payload]
  (let [incident (:incident payload)
        started_at (* 1000 (:started_at incident))
        ended_at (when-let [ended_at (:started_at incident)] (* 1000 ended_at))
        ...
        facts (cond-> [{:title "Incident ID" :value incident_id}
                ...
                resource_name (conj {:type "TextBlock" :value (str "Resource name" resource_name)})
  ...
  (let [auth-token (.. js/process -env -AUTH_TOKEN)
    ...
    (if (= auth-token req-auth-token)
      (let [payload (-> (.-body req) to-clj)
            {:keys [incident_id started_at]} payload
            payload-valid (and incident_id started_at)]
        (cond
